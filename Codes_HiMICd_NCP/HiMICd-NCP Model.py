...
if target_variable in variable_column_mapping:
    ...
    feature_matrix[feature_column_name] = year_filtered_data.iloc[:, column_index]
...
final_n_estimators = len(final_cv_output['rmse-mean'])
final_rmse = final_cv_output['rmse-mean'][-1]
...
training_dataset['year'] = pd.to_datetime(training_dataset['time'], errors='coerce').dt.year
...
testing_dataset['year'] = pd.to_datetime(testing_dataset['time'], errors='coerce').dt.year
testing_dataset['month'] = pd.to_datetime(testing_dataset['time'], errors='coerce').dt.month
testing_dataset['day'] = pd.to_datetime(testing_dataset['time'], errors='coerce').dt.day
...
if TARGET_VARIABLE in variable_column_map:
    ...
    training_features[feature_col_name] = training_year_filtered.iloc[:, col_idx]
...
if TARGET_VARIABLE in variable_column_map:
    col_idx = variable_column_map[TARGET_VARIABLE]
    feature_col_name = TARGET_VARIABLE.lower()
    testing_features[feature_col_name] = testing_year_filtered.iloc[:, col_idx]
...
if TARGET_VARIABLE in variable_column_mapping:
    column_index = variable_column_mapping[TARGET_VARIABLE]
    feature_column_name = TARGET_VARIABLE.lower()
    training_features[feature_column_name] = training_year_data.iloc[:, column_index]
...
if TARGET_VARIABLE in variable_column_mapping:
    column_index = variable_column_mapping[TARGET_VARIABLE]
    feature_column_name = TARGET_VARIABLE.lower()
    testing_features[feature_column_name] = testing_year_data.iloc[:, column_index]
...
for file_index, input_file_path in enumerate(input_files_list):
    ...
    input_file_directory = input_file_path.split("/")[5]

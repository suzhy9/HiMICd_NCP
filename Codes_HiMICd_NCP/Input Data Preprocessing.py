# %%[1 Data Acquisition - by Google Earth Engine]
...
raw_data_filepath = r"E:\high-resolution atmospheric moisture\Data\1 Allcombined_cleanedData\CombinedALLData2003_2020.csv"
dataset = pd.read_csv(raw_data_filepath)

# Remove unnamed columns
unnamed_columns = dataset.filter(regex="Unname")
cleaned_dataset = dataset.drop(unnamed_columns, axis=1)

feature_columns = cleaned_dataset.columns[1:]
print(f"Dataset dimensions: {cleaned_dataset.shape}")

###------ Data Cleaning Procedure ------
# Filter out invalid values (-9999.0)
for column_name in feature_columns:
    cleaned_dataset = cleaned_dataset.loc[~cleaned_dataset[column_name].isin([-9999.0])]
    print(f"Removed -9999 values from {column_name}: remaining records - {cleaned_dataset.shape}")

# Remove rows with missing values
for feature in feature_columns:
    cleaned_dataset = cleaned_dataset.dropna(axis='index', how='any', subset=[feature])
print(f"After removing NaN values: final dataset size - {cleaned_dataset.shape}")

###------ Data Export ------
output_filepath = r"E:\high-resolution atmospheric moisture\Data\1 Allcombined_cleanedData\Cleaned_ALLData2003_2020.csv"
cleaned_dataset.to_csv(output_filepath, encoding='utf-8', index=False)
...
target_year = 2003 # from 2003 to 2020

cleaned_dataset_path = r"E:\high-resolution atmospheric moisture\Data\1 Allcombined_cleanedData\Cleaned_ALLData2003_2020.csv"
processed_data = pd.read_csv(cleaned_dataset_path)

# Remove any unnamed columns
unnamed_columns = processed_data.filter(regex="Unname")
processed_data = processed_data.drop(unnamed_columns, axis=1)

# Clean up memory
del unnamed_columns, cleaned_dataset_path

###------ Dataset Splitting Procedure ------
validation_ratio = 0.2
station_ids = np.unique(processed_data['id'])
...
# Extract data for current station
  station_data = processed_data.loc[processed_data["id"] == station_id]

# Split into training and validation sets
station_train, station_validation = train_test_split(
  station_data,
  test_size=validation_ratio,
  random_state=42
)

# Combine with overall datasets
training_dataset = pd.concat([training_dataset, station_train], axis=0, ignore_index=False)
validation_dataset = pd.concat([validation_dataset, station_validation], axis=0, ignore_index=False)

print(f"Processed station {processing_counter}: ID {station_id}")
processing_counter += 1

###------ Results Summary and Export ------
print(f"Original dataset size: {processed_data.shape}")
print(f"Training dataset size: {training_dataset.shape}")
print(f"Validation dataset size: {validation_dataset.shape}")

# Export the split datasets
training_output_path = r"E:\high-resolution atmospheric moisture\Data\2 Split Data\Train_all.csv"
validation_output_path = r"E:\high-resolution atmospheric moisture\Data\2 Split Data\Test_all.csv"

training_dataset.to_csv(training_output_path, encoding='utf-8', index=False)
validation_dataset.to_csv(validation_output_path, encoding='utf-8', index=False)